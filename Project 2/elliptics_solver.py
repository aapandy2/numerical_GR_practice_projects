...
def jacobian(f_n):
	...
	for i in range(N):
		if(i == 0): #r=0 BC: psi'=0
			...
			A[i, :] = [( -2./delta_r**2. + 5.*psi[i%N]**4./12. 
	                           ...
	                           + np.pi*(xi[n, i%N]**2. + Pi[n, i%N]**2.) ) if j==i
	...
	for i in range(2*N, 3*N):
		if(i == 2*N): #r=0 BC: alpha' = 0
			...
			A[i, :] = [-2./delta_r**2. - (-1.)*alpha[i%N]**(-2.)*(2.*psi[i%N]**4./3. * ( (beta[i%N+1] - beta[i%N-1])/(2.*delta_r) - beta[i%N]/r_grid[i%N]  )**2.) - 8. * np.pi * Pi[n, i%N] if j==i
...
Ainv = np.linalg.inv(jacobian(f_n))
...
def residual(f_n):
	...
	for j in range(N):
		if(j == 0): #r=0 BC: psi'=0
			...
			ans[j] = ( (psi[j%N+1] - 2.*psi[j%N] + psi[j%N-1])/delta_r**2. + (psi[j%N+1]-psi[j%N-1])/(2.*delta_r)*(2./r_grid[j%N]) + psi[j%N]**5./12. * (1./alpha[j%N] * ( (beta[j%N+1]-beta[j%N-1])/(2.*delta_r) - beta[j%N]/r_grid[j%N]  )  )**2. + np.pi * psi[j%N] * (xi[n, j%N]**2. + Pi[n, j%N]**2.) )
	...
	for j in range(N, 2*N):
		if(j == N): #r=0 BC: beta = 0
			...
			ans[j] = ( (beta[j%N+1] - 2.*beta[j%N] + beta[j%N-1])/delta_r**2. + ( (beta[j%N+1]-beta[j%N-1])/(2.*delta_r) - beta[j%N]/r_grid[j%N]  ) * (2./r_grid[j%N] + 6.*(psi[j%N+1]-psi[j%N-1])/(2.*delta_r*psi[j%N]) - (alpha[j%N+1]-alpha[j%N-1])/(2.*delta_r*alpha[j%N])) + 12.*np.pi*alpha[j%N]*xi[n, j%N]*Pi[n, j%N]/psi[j%N]**2. )
	...
	for j in range(2*N, 3*N):
		if(j == 2*N): #r=0 BC: alpha' = 0
			...
			ans[j] = ( (alpha[j%N+1] - 2.*alpha[j%N] + alpha[j%N-1])/delta_r**2. + (alpha[j%N+1]-alpha[j%N-1])/(2.*delta_r) * (2./r_grid[j%N] + 2.*(psi[j%N+1] - psi[j%N-1])/(2.*delta_r)) - alpha[j%N]**(-1.)*(2*psi[j%N]**4./3. * ( (beta[j%N+1]-beta[j%N-1])/(2.*delta_r) - beta[j%N]/r_grid[j%N] )**2. ) - 8.*np.pi*alpha[j%N]*Pi[n, j%N] )

	return ans


print 'solve nonlinear system'
print 'iteration = 0', 'max(residual) =', np.amax(residual(f_n))
tolerance    = 1e-5
res          = 1.
iteration    = 0
max_iter     = 1000
...
while(np.abs(np.amax(res)) > tolerance and iteration < max_iter):
	...
	print 'iteration =', iteration, 'max(residual) =', np.amax(res)

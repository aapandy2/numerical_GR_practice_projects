...
\section{Problem 7}
We are now asked to compute the Ricci scalar, which may be related to the trace of the stress-energy tensor via a contraction of the Einstein equation:
\begin{equation}
R = - 8 \pi T.
\end{equation}
Thus, if we compute the trace of the stress-energy tensor we may trivially obtain the Riemann scalar.  Fortunately in the appendix we compute a number of components required to compute $T$: equation \ref{eq:rho} is $\alpha^2 T^{tt}$, equation \ref{eq:S_rr} gives us $\psi^8 T^{rr}$, equation \ref{eq:S_thetatheta} gives us $\psi^8 r^4 T^{\theta \theta}$, and equation \ref{eq:S_phiphi} gives us $\psi^8 r^4 \sin^4 \theta T^{\phi \phi}$.  All we have left to compute is $T^{tr} = T^{rt}$, which is:
\begin{equation}
\begin{aligned}
T^{tr} &= -\frac{\beta  \dot{\Phi}^2}{2 \alpha ^4}-\frac{\dot{\Phi} \Phi' \left(\frac{1}{\psi ^4}-\frac{\beta ^2}{\alpha ^2}\right)}{\alpha ^2}+\frac{\beta  \Phi'^2 \left(\frac{1}{\psi ^4}-\frac{\beta ^2}{\alpha ^2}\right)}{2 \alpha ^2} \\
&= \frac{\beta  \xi^2}{\psi ^4 2 \alpha ^2} - \frac{\dot{\Phi} \xi}{\alpha^2 \psi ^4} - \frac{1}{2} \Big( \frac{\beta  \dot{\Phi}^2}{\alpha ^4} - \frac{2 \dot{\Phi} \xi \beta ^2}{\alpha ^4} + \frac{\beta  \xi^2 \beta ^2}{\alpha^4} \Big) \\
&= \frac{\beta  \xi^2}{\psi ^4 2 \alpha ^2} - \frac{\dot{\Phi} \xi}{\alpha^2 \psi ^4} - \frac{\beta}{2 \alpha^4} \Big( \dot{\Phi}^2 - 2 \dot{\Phi} \xi \beta + \xi^2 \beta ^2 \Big) \\
&= \frac{\beta  \xi^2}{\psi ^4 2 \alpha ^2} - \frac{\dot{\Phi} \xi}{\alpha^2 \psi ^4} - \frac{\beta}{2 \alpha^4} \frac{\alpha^2}{\psi^4} \Pi^2 \\
&= \frac{\beta  \xi^2}{\psi ^4 2 \alpha ^2} - \frac{\beta}{2 \alpha^4} \frac{\alpha^2}{\psi^4} \Pi^2 - \frac{\dot{\Phi} \xi}{\alpha^2 \psi ^4} \\
&= \frac{\beta  \xi^2}{\psi ^4 2 \alpha ^2} - \frac{\beta}{2 \alpha^4} \frac{\alpha^2}{\psi^4} \Pi^2 - \frac{\xi}{\alpha^2 \psi ^4} \Big[ \frac{\alpha \Pi}{\psi^2} + \beta \xi \Big] \\
&= -\frac{\beta  \xi^2}{\psi ^4 2 \alpha ^2} - \frac{\beta \Pi^2}{2 \alpha^2 \psi^4} - \frac{\xi \Pi}{\alpha \psi^6}. \\
\end{aligned}
\end{equation}
Now we can collect the other terms from the aforementioned equations in the appendix:
\begin{align}
T^{tt} &= \frac{1}{2 \alpha^2 \psi^{4}} \Big( \Pi^2 + \xi^2 \Big) \\
T^{rr} &= \frac{1}{2 \psi^8} \Big( \xi^2 + \Pi^2 \Big) \\
T^{\theta \theta} &= \frac{1}{2 \psi^8 r^2} \Big( \Pi^2 - \xi^2 \Big) \\
T^{\phi \phi} &= \frac{1}{2 \psi^8 r^2 \sin^2 \theta} \Big( \Pi^2 - \xi^2 \Big).
\end{align}
Now we can finally compute the trace
\begin{equation}
\begin{aligned}
T &= g_{tt} T^{tt} + g_{tr} T^{tr} + g_{rt} T^{rt} + g_{rr} T^{rr} + g_{\theta \theta} T^{\theta \theta} + g_{\phi \phi} T^{\phi \phi} \\
&= 
\end{aligned}
\end{equation}

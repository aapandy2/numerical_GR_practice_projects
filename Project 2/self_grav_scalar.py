...
#		B[i, :] = [0 for j in range(3*N)]
	    	...
	    	B[i, :] = [1. + delta_t/2. * ( beta[n][i%N]/(3.*r_grid[i%N])
	                                                + (beta[n][i%N+1]-beta[n][i%N-1])/(6. * 2. * delta_r) ) - epsilon/(16.*1.)*(6.+1.) if j==i
	                       else delta_t/2. * beta[n][i%N]/(2.*delta_r) - epsilon/(16.*1)*(-4.)  if j==i+1
	                       else -delta_t/2. * beta[n][i%N]/(2.*delta_r) - epsilon/(16.*1)*(-4.)  if j==i-1
	    	    	       else - epsilon/(16.*1.)*1. if j==i+2
	                       else 0 for j in range(3*N)]
...
def update_r_s(ans, timestep):
    for i in range(3*N):
        if(i < N):
            ...
            psi_evo[timestep, i-2*N] = ans[i]

    return 0

#initialize residuals
elliptic_res     = np.zeros(3*N)
matter_res       = np.zeros(3*N)
...
def solve_system(n):
	...
	while(num_iter <= max_num_iter and normres > tol):
		...
		populate_matrices(n)            #A has n+1, B has n
                u = update_u(n)                 #populate u with xi, Pi, psi_evo at n = 0
                bb = B.dot(u)                   #compute bb (all at n = 0)
		ans = np.linalg.solve(A, bb+c)  #solve for xi, Pi, psi_evo at n=1 with elliptics guess for n=1
		update_r_s(ans, n+1)            #update xi, Pi, psi_evo at n=1 with elliptics guess for n=1
		...
		if(GEOM_COUPLING == True):
			...
			if(PSI_EVOL == True):
                        	psi[n+1, :] = psi_evo[n+1, :] #TODO: check this
			...
			while(num_iter_elliptic <= max_num_iter and elliptic_res_norm > tol):
				...
				#if PSI_EVOL == True, then we have psi at the advanced timestep from
		...
		xi_residual, Pi_residual, psi_evo_residual = matter_residuals(xi, Pi, psi, beta, alpha, r_grid, n, delta_t, epsilon)
		matter_res = np.append(xi_residual, [Pi_residual, psi_evo_residual])

                if(PSI_EVOL == False):
                    res = np.append(elliptic_res, matter_res[0:2*N])
                else:
                    res = np.append(elliptic_res[N:3*N], matter_res)
		normres = np.amax(np.abs(res))
		print 'inf. norm of full residual:', np.amax(np.abs(elliptic_res[N:3*N])), np.amax(np.abs(matter_res[0:3*N])), normres
...
psi_evo_residual = np.zeros((timesteps, N))
...
for n in range(timesteps-1):
	...
	xi_residual[n, :], Pi_residual[n, :], psi_evo_residual[n, :] = matter_residuals(xi, Pi, psi, beta, alpha, r_grid, n, delta_t, epsilon) 
...
#			psi_evo_residual[n, j] = ( -psi[n+1,j+2] + 4.*psi[n+1,j+1] - 3.*psi[n+1,j]
...
#                                            )
#                        psi_evo_residual[n,j] = ( (psi[n+1,j]-psi[n,j])/delta_t
...
#                                                    + Pi[n,j]/r_grid[j]) )
#			psi_evo_residual[n,j] = ( (3.*psi[n,j]-4.*psi[n,j-1]+psi[n,j-2])/(2.*delta_r) + psi[n,j]/r_grid[j]
...
np.savetxt('psi_evo_residual.txt', psi_evo_residual)
